...
```
Read("{SKILL_PATH}/references/common-rules.md")
```
...
```
Read("{SKILL_PATH}/references/{对应专项规范文件}")
```
...
主流程在 prompt 中要求本 agent 先 Read 本文件（`.agents/skills/diagram-generator/references/drawer.md`），再按 Step 1-5 执行。
