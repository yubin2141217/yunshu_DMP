...
| 业务场景 | 专项规范文件 | 模板文件（`examples/` 根目录） |
|---------|------------|---------|
| 业务流程图（泳道图、BPMN） | `business-flow-rules.md` | `swimlane.xml`、`cross-functional.xml`、`matrix-swimlane.xml`、`bpmn-flow.xml` |
| 用户操作流程图 | `operation-flow-rules.md` | `flowchart.xml` |
| 系统架构图 | `system-architecture-rules.md` | `architecture.xml`、`system-arch.xml`、`orgchart.xml` |
| UML图（用例、时序、类图、状态、ER） | `uml-rules.md` | `sequence.xml`、`uml-class.xml`、`uml-usecase.xml`、`uml-state.xml`、`er-diagram.xml` |
