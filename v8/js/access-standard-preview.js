  ...
  window.YunshuStandardPreviewDrawer = {
    ...
    data() {
      return {
        ...
        apiParamRows,
        metaTab: 'table',
        categoryLabels: (window.YunshuMock && window.YunshuMock.categoryLabels) || {
          table: '库表数据集',
          unstructured: '非结构化数据',
          api: '接口数据',
        },
        requiredLabels: (window.YunshuMock && window.YunshuMock.requiredLabels) || {
          required: '必接',
          suggested: '建议',
          optional: '可选',
        },
    ...
    computed: {
      drawerTitle() {
        return this.fileName ? '预览接入规范 · ' + this.fileName : '预览接入规范'
      },
      frontText() {
    ...
    methods: {
      ...
      categoryLabel(v) {
        return this.categoryLabels[v] || v || '—'
      },
      requiredLabel(v) {
        return this.requiredLabels[v] || v || '—'
      },
    },
    template: `
      <a-drawer
      ...
      >
        <div class="profile-page" style="margin: 0">
          <p class="profile-desc-text" style="margin-top: 0">
            以下为当前启用接入规范的完整内容：数据标准约定「接什么」，接入方案约定「怎么接」，请一并转交供数方。
          </p>
          <template v-if="metadata">
            <div class="profile-section-title">数据标准 · 业务属性</div>
            <a-descriptions :column="2" size="large" class="profile-descriptions profile-descriptions--wide">
              <a-descriptions-item label="中文名称">{{ metadata.name }}</a-descriptions-item>
              <a-descriptions-item label="编码">{{ metadata.code }}</a-descriptions-item>
              <a-descriptions-item label="数据分类">{{ categoryLabel(metadata.category) }}</a-descriptions-item>
              <a-descriptions-item label="主题">{{ metadata.topic || '—' }}</a-descriptions-item>
              <a-descriptions-item label="是否必接">{{ requiredLabel(metadata.requiredLevel) }}</a-descriptions-item>
              <a-descriptions-item label="提供方">{{ metadata.provider || '—' }}</a-descriptions-item>
              <a-descriptions-item label="业务口径" :span="2">{{ metadata.bizCaliber || '—' }}</a-descriptions-item>
            </a-descriptions>
            <div class="profile-section-title" style="margin-top: 20px">数据标准 · 技术属性</div>
            <a-descriptions :column="2" size="large" class="profile-descriptions profile-descriptions--wide">
              <a-descriptions-item label="字符集">{{ metadata.charset || '—' }}</a-descriptions-item>
              <a-descriptions-item label="主键">{{ metadata.primaryKey || '—' }}</a-descriptions-item>
              <a-descriptions-item label="数据类型">{{ metadata.dataType || '—' }}</a-descriptions-item>
              <a-descriptions-item label="更新周期">{{ metadata.updateCycle || '—' }}</a-descriptions-item>
              <a-descriptions-item label="大小上限" :span="2">{{ metadata.maxSize || '—' }}</a-descriptions-item>
            </a-descriptions>
            <div class="profile-section-title" style="margin-top: 20px">数据标准 · 运维操作属性</div>
            <a-descriptions :column="2" size="large" class="profile-descriptions profile-descriptions--wide">
              <a-descriptions-item label="负责人">{{ metadata.owner || '—' }}</a-descriptions-item>
              <a-descriptions-item label="状态">{{ metadata.status === 'enabled' ? '启用' : '停用' }}</a-descriptions-item>
              <a-descriptions-item label="监控方式" :span="2">{{ metadata.monitor || '—' }}</a-descriptions-item>
              <a-descriptions-item label="时效/SLA" :span="2">{{ metadata.sla || '—' }}</a-descriptions-item>
              <a-descriptions-item label="备注" :span="2">{{ metadata.remark || '—' }}</a-descriptions-item>
              <a-descriptions-item label="更新时间">{{ metadata.updatedAt || '—' }}</a-descriptions-item>
            </a-descriptions>
          </template>
          <template v-if="scheme">
            <div class="profile-section-title" style="margin-top: 20px">接入方案 · 对接策略</div>
            <a-descriptions :column="2" size="large" class="profile-descriptions profile-descriptions--wide">
          ...
          <div class="profile-section-title" style="margin-top: 20px">字段与分类说明 · 需要接入的数据</div>
