...
<script setup lang="ts">
  ...
  /**
   * 递归过滤菜单路由，移除隐藏的菜单项
   * 一级菜单（isFirstLevel）在子路由全部隐藏时仍保留，并作为叶子节点跳转父 path
   * 非一级：若所有子菜单都被隐藏，则父菜单也会被隐藏
   * @param items 菜单项数组
   * @returns 过滤后的菜单项数组
   */
  const filterRoutes = (items: AppRouteRecord[]): AppRouteRecord[] => {
    return items
      ...
      .map((item) => {
        ...
        const filteredChildren = filterRoutes(item.children)
        // 一级菜单且无可见子菜单：作为叶子，点击跳转自身 path（如 /product、/trial）
        if (item.meta?.isFirstLevel && filteredChildren.length === 0) {
          return { ...item, children: undefined }
        }
  ...
  const getMenuIndex = (item: AppRouteRecord): string => {
    if (isExternalLink(item)) {
      return item.meta?.link || item.path || item.meta?.title || ''
    }
