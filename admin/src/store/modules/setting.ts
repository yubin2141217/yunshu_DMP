...
const {
  ...
  forceApplyDefaultFrameworkOnStartup
...
export const useSettingStore = defineStore(
  ...
  () => {
    ...
    // 框架布局设置（初始值来自系统配置；若开启强制默认，则不持久化该项，每次启动均为 defaultFrameworkType）
    /** 框架类型 */
    const frameworkType = ref(defaultFrameworkType)
    ...
    const initThemeStyles = () => {
      // 迁移旧默认主色 / 圆角到设计稿令牌
      if (systemThemeColor.value === '#165DFF') {
        systemThemeColor.value = AppConfig.systemMainColor[0]
      }
      if (customRadius.value === '0.75') {
        customRadius.value = AppConfig.systemSetting.defaultCustomRadius
      }
  ...
  },
  {
    persist: {
      ...
      ...(forceApplyDefaultFrameworkOnStartup ? { omit: ['frameworkType'] as const } : {})
    }
  }
)
