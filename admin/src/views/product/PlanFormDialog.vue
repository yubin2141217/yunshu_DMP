      ...
      <ElForm ref="formRef" :model="form" :rules="rules" label-width="176px" class="plan-form">
        <ElFormItem label="所属产品" prop="productId">
          <ElSelect
            ...
            filterable
            placeholder="请选择产品"
            :disabled="Boolean(productId) || Boolean(planId)"
        ...
        <ElFormItem label="附加配置">
          <div class="custom-config-block">
            ...
            <div v-for="(item, idx) in form.customConfigs" :key="idx" class="custom-config-card">
              ...
              <div v-if="item.control === 'radio' || item.control === 'checkbox'" class="options-block">
...
<script setup lang="ts">
  ...
  defineOptions({ name: 'PlanFormDialog' })
  ...
  function normalizeLoadedConfig(item: Partial<CustomConfigItem> & { control?: string }): CustomConfigItem {
    const control: CustomConfigControl =
      item.control === 'radio' || item.control === 'checkbox' ? item.control : 'input'
    const options =
      control === 'input' ? [] : item.options?.length ? [...item.options] : ['']
  ...
  function resetForm() {
    form.id = 0
    form.productId = props.productId || undefined
    form.name = ''
    form.intro = ''
    form.displayStatus = 1
    form.trialTimeType = '1m'
    form.trialDays = 30
    form.extensionType = '0'
    form.extensionCount = 4
    form.freezeDays = []
    form.allowRepeatApply = false
    form.handler = ''
    form.accessApi = ''
    form.extendConfig = ''
    form.stopConfig = ''
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const res = props.planId
        ? await updatePlan(payload as { id: number })
        : await createPlan(payload)
      if (res.code === 200) {
        ElMessage.success(props.planId ? '方案更新成功' : '方案创建成功')
  ...
  watch(
    ...
    async (val) => {
      ...
      if (props.planId) {
        const res = await fetchPlanDetail(props.planId)
